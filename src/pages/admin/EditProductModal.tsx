...
export const EditProductModal = ({ isOpen, onClose, product }: EditProductModalProps) => {
    ...
    useEffect(() => {
        if (product && isOpen) {
            ...
            setColors((product.variants?.colors || []).join(', '));
                                ...
                                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        ...
                                        <div className="flex flex-col gap-1.5">
                                            ...
                                            <input value={colors} onChange={e => setColors(e.target.value)} type="text" placeholder="Black, Crimson, Void" className="w-full border border-slate-300 rounded-lg p-2.5 text-sm font-medium text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all shadow-sm" />
                                        </div>
                                    </div>
                                        ...
                                        {imageFiles.length > 0 ? (
                                            ...
                                            <div className="flex flex-wrap gap-3 justify-center pointer-events-none">
                                                {existingImages.map((src, i) => (
                                                    <div key={i} className="relative w-20 h-20 border border-slate-200 rounded overflow-hidden shadow-sm bg-white">
                                                        <img src={src} className="w-full h-full object-cover" alt="existing" />
                                                    </div>
                                                ))}
                                                <div className="w-full text-center mt-3 text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center justify-center gap-1.5">
